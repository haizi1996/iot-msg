...
public class MqttConnectionFactory extends DefaultConnectionFactory {
    public MqttConnectionFactory(ConcurrentHashMap<MqttMessageType, UserProcessor<?>> userProcessors, ConfigurableInstance confInstance , MessageHandler messageHandler) {
        super(new MqttCoder(), new HeartbeatHandler(), new RpcHandler(userProcessors , messageHandler),
                confInstance);
    }
}
